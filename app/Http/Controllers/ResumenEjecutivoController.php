...
class ResumenEjecutivoController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        $refrendosData = ['contratos' => 0, 'prestamo' => 0];
        $desempenosData = ['contratos' => 0, 'prestamo' => 0];
        ...
        foreach ($sucursalesSeleccionadas as $sucursal) {
            ...
            try {
                ...
                // 3. EMPEÑOS, REFRENDOS, DESEMPEÑOS
                $movimientosResult = DB::connection($connectionName)->selectOne("
                    SELECT
                        COUNT(DISTINCT CASE WHEN mo.cod_tipo_movimiento = 1 THEN con.contrato ELSE NULL END) as contratos_empeno,
                        COALESCE(SUM(CASE WHEN mo.cod_tipo_movimiento = 1 THEN con.prestamo ELSE 0 END), 0) as total_empeno,
                        COUNT(DISTINCT CASE WHEN mo.cod_tipo_movimiento = 2 THEN con.contrato ELSE NULL END) as contratos_refrendo,
                        COALESCE(SUM(CASE WHEN mo.cod_tipo_movimiento = 2 THEN con.prestamo ELSE 0 END), 0) as total_refrendo,
                        COUNT(DISTINCT CASE WHEN mo.cod_tipo_movimiento = 4 THEN con.contrato ELSE NULL END) as contratos_desempeno,
                        COALESCE(SUM(CASE WHEN mo.cod_tipo_movimiento = 4 THEN con.prestamo ELSE 0 END), 0) as total_desempeno
                    FROM movimientos mo
                ...
                $totalGastosGlobal = $totalEgresos; // Fixing an undefined variable warning

                $empenosData['contratos'] += (int)($movimientosResult->contratos_empeno ?? 0);
                $empenosData['prestamo'] += (float)($movimientosResult->total_empeno ?? 0);

                $refrendosData['contratos'] += (int)($movimientosResult->contratos_refrendo ?? 0);
                $refrendosData['prestamo'] += (float)($movimientosResult->total_refrendo ?? 0);

                $desempenosData['contratos'] += (int)($movimientosResult->contratos_desempeno ?? 0);
                $desempenosData['prestamo'] += (float)($movimientosResult->total_desempeno ?? 0);
        ...
        return response()->json([
            ...
            'refrendosData' => $refrendosData,
            'desempenosData' => $desempenosData,
